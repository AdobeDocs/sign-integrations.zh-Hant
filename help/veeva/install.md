...
| file_hash__c | 檔案雜湊 | 字串 （50） | 對已傳送至 Adobe Sign 的檔案保持 md5 chcksum |
...
若要成功整合保存庫，系統會建立名為「Adobe Sign整合設定檔」的新安全設定檔， ** 並為 *Adobe Sign管理員動作設定許可權* 。 Adobe Sign整合設定檔會指派給系統帳戶，並會在呼叫保存 API 時由整合使用。 此描述檔允許下列許可權：
...
若要設定 [!DNL Vault] Adobe Sign，系統會建立一個名為 *「Adobe Sign管理員群組」的新群組* 。 此群組是用來為Adobe Sign相關欄位設定檔欄位層級安全性， *且預設應包含Adobe Sign整合* 描述檔」。
...
這些共用欄位必須新增至所有符合Adobe簽名資格的檔案類型。 這兩個欄位皆應具有特定安全性，僅允許Adobe Sign管理員群組的成員更新其值。
...
將保存檔傳送給Adobe Sign時，其狀態應對應至合約的狀態。 完成此步驟時，請在符合Adobe簽名資格的檔所使用的每個生命週期中新增以下狀態：
...
* **在「草稿** Adobe Sign：這是狀態的預留位置名稱，表示檔已上傳至Adobe Sign，且其合約處於「草稿」狀態。 這是必要的狀態。 此狀態必須為下列五個使用者動作加以凹陷：
   ...
   * 稱為「取消Adobe Sign」網頁動作。 這個狀態必須具備安全性，才能Adobe Sign管理員角色：檢視檔、檢視內容、編輯欄位、編輯關係、下載來源、管理可檢視轉譯，以及變更狀態。
...
* **在「Adobe Sign編寫」中** ：這是狀態的預留位置名稱，表示檔已上傳至 Adobe Sign，且其合約處於 AUTHORING 或DOCUMENTS_NOT_YET_PROCESSED狀態。 這是必要的狀態。 此狀態必須已定義下列四個使用者動作：
   ...
   * 將檔狀態變更為「在Adobe簽署狀態」的動作。 無論生命週期如何，此使用者動作的名稱對所有檔案類型都必須相同。 如果需要，此動作的標準可以設定為「允許Adobe Sign使用者動作等於是。
   * 稱為「網路動作」的動作「Adobe Sign」
   * 稱為「取消Adobe Sign」網頁動作。 這個狀態必須具備安全性，才能Adobe Sign管理員角色：檢視檔、檢視內容、編輯欄位、編輯關係、下載來源、管理可檢視轉譯，以及變更狀態。

   ![生命週期狀態 3 的影像](images/lifecycle-state3.png)

* **在Adobe簽署中** ：這是狀態的預留位置名稱，表示檔已上傳至Adobe Sign，且其合約已傳送給參與者 （OUT_FOR_SIGNATURE 或OUT_FOR_APPROVAL狀態）。 這是必要的狀態。 此狀態必須已定義下列五個使用者動作：
   ...
   * 稱為「網頁動作」的動作 *Adobe Sign* 。
   * 稱為「網頁動作 *取消」的動作Adobe Sign* 。 這個狀態必須具備安全性，才能Adobe Sign管理員角色：檢視檔、檢視內容、編輯欄位、編輯關係、下載來源、管理可檢視轉譯，以及變更狀態。
...
管理員必須建立名為「Adobe Sign檔」的新「檔案類型群組」記錄。 為所有符合Adobe Sign程式資格的檔分類新增此檔案類型群組。 由於檔案類型群組屬性不會從類型繼承為子類型，也不會從子類型繼承到分類層級，因此必須針對每個符合Adobe Sign資格的檔分類進行設定。
...
1. [前往「首頁」的Adobe Sign  [!DNL Veeva Vault]  頁 ](https://static.adobesigncdn.com/veevavaultintsvc/index.html) 。
1. **** 選取右上角的「登入」。
...
1. 在開啟的「Adobe Sign」登入頁面中，提供帳戶管理員電子郵件和密碼，然後選取 **[!UICONTROL 「按一下]** 」。
   ...
   使用者登入後，頁面會在右上角顯示關聯的電子郵件 ID，以及額外的「設定」索引標籤 ，如下圖所示。
...
1. 選 **[!UICONTROL 取「儲存]** 」以儲存新的連線。
   ...
   新連線會顯示在「設定」索引標籤下方，顯示已成功整合 [!DNL Veeva Vault] 至Adobe Sign。
...
**步驟 5.** 使用安全性描述檔「Adobe Sign整合描述檔」建立整合使用者設定檔，並將其指派給 Adobe Sign Admin Group。
...
**步驟 11.** 所有設定都完成後，系統應確保 DAC 為所有符合Adobe Sign程式的檔新增Adobe Sign管理員使用者。 建立適當的「使用者角色設定」記錄，將「檔案類型群組」指定為「Adobe Sign檔」、「應用程式角色」指定為「Adobe Sign管理員角色」，以及整合使用者，即可完成此作業。
