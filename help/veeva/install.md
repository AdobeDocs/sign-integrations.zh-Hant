...
| file_hash__c | 檔案雜湊 | 字串 （50） | 在已傳送給 Adobe Sign 的檔案中，保持 md5 chcksum |
...
若要成功整合保存庫，系統會建立名為「Adobe Sign 整合設定檔」的新安全設定檔， ** 並為 *「Adobe Sign管理員動作」設定許可權* 。 Adobe Sign整合設定檔會指派給系統帳戶，並會在呼叫保存 API 時由整合使用。 此描述檔允許下列許可權：
...
若要設定 [!DNL Vault] Adobe Sign，系統會建立一個名為 *「Adobe Sign Admin Group」的新群組* 。 此群組是用來為Adobe Sign相關欄位設定檔欄位層級安全性， ** 並且預設包含Adobe Sign整合描述檔」。
...
* **在Adobe簽署之前** （已審核）：這是可傳送檔至Adobe Sign狀態的預留位置名稱。 根據檔案類型，檔案類型可以是「草稿」狀態或「已審核」。 檔狀態標籤可根據客戶需求自訂。 在Adobe簽署狀態之前，必須定義下列兩個使用者動作：

   * 將檔狀態變更為「 *在草稿中Adobe Sign* 狀態的動作。 對於任何生命週期的所有檔案類型，此使用者動作的名稱必須相同。 如果需要，此動作的標準可以設定為「允許Adobe Sign使用者動作等於是。」
...
* **在「草稿Adobe Sign** 中：這是狀態的預留位置名稱，表示檔已上傳至Adobe Sign，且其合約處於「草稿」狀態。 這是必要的狀態。 此狀態必須為下列五個使用者動作加以凹陷：
   ...
   * 稱為「取消Adobe Sign」的動作。 這個狀態必須具備可讓Adobe Sign管理員角色：檢視檔、檢視內容、編輯欄位、編輯關係、下載來源、管理可檢視轉譯，以及變更狀態。
...
管理員需要建立名為「Adobe Sign檔」的新「檔案類型群組」記錄。 為所有符合Adobe Sign程式資格的檔分類新增此檔案類型群組。 由於檔案類型群組屬性不會從類型繼承為子類型，也不會從子類型繼承到分類層級，因此必須針對每個符合Adobe Sign資格的檔分類進行設定。
